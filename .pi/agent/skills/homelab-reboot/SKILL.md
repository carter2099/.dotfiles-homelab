...
4. **Reboot.** Run `sudo systemctl reboot`. The session will drop immediately. The systemd unit `pi-web.service` will come back online automatically once the host is back. When the user opens a new session, the startup-check in `AGENTS.md` will surface the state file.
