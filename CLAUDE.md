...
- **Service unit:** `~/.config/systemd/user/opencode-homelab.service` → `opencode web --hostname 0.0.0.0 --port 48099` (binds `0.0.0.0` so the k3s Traefik can reach it via the host LAN IP; see below).
...
- **Two independent credentials (defense in depth):**
  ...
  2. **`OPENCODE_SERVER_PASSWORD`** — opencode's own HTTP basic-auth, enforced server-side (401 without). Stored in `~/.config/opencode-homelab/env` (gitignored, 600). **Never commit this file.**
- **Routing:** follows the house Traefik pattern, *not* the direct webhook pattern. Tunnel ingress `opencode.carter2099.com → http://localhost:80` → Traefik → k3s `opencode-external-service` (ExternalService + Endpoints → `192.168.4.92:48099`) → the host `opencode web` process. Manifests: `~/k3s/opencode/deploy.yaml` (mirrors `delta_neutral`/`blog`). DNS: proxied CNAME `opencode` → `<tunnel-id>.cfargotunnel.com`.
- **Known residual:** because Traefik reaches host apps over the LAN IP (not loopback), the `:48099` port is bound `0.0.0.0` and is therefore reachable on the LAN directly, bypassing CF Access — but still gated by `OPENCODE_SERVER_PASSWORD`. Optional future hardening: firewall-scope `48099` to loopback + the k3s pod CIDR only.
