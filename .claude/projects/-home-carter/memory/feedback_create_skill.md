...
description: When creating a new agent skill, invoke the create-skill skill instead of writing files directly — it handles dotfiles tracking automatically.
type: feedback
originSessionId: 39399a38-486b-4d35-8579-01557ebf45d5
---
Always use the `/create-skill` skill when creating a new user-level skill. Writing a skill file directly (under `~/.config/opencode/skills/*/SKILL.md` or `.opencode/skills/*/SKILL.md`) skips the `dotfiles add` + commit + push step, leaving the skill untracked and at risk of being lost if homelab storage is wiped.
