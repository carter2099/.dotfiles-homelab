...
- ✅ **Remote-control replacement built 2026-06-11** — `opencode-homelab.service` runs `opencode web --hostname 0.0.0.0 --port 48099`, reachable at `https://opencode.carter2099.com`. **Additive** (Claude daemon still runs in parallel) and **intentionally full-privilege** (Carter's call: parity with the Claude daemon, trust anchor just moves from Anthropic OAuth → Cloudflare Access — he explicitly rejected stripping sudo/NoNewPrivileges). Two independent credentials: CF Access SSO at the edge (verified: unauth → 302 to carter2099.cloudflareaccess.com, never reaches host) + `OPENCODE_SERVER_PASSWORD` basic auth (in gitignored `~/.config/opencode-homelab/env`). Routing uses the **house Traefik pattern** (Carter chose it over the direct webhook pattern for consistency): tunnel `opencode.carter2099.com → localhost:80` → Traefik → k3s `opencode-external-service` (ExternalService+Endpoints → 192.168.4.92:48099) → host process. Manifests `~/k3s/opencode/deploy.yaml`. **Use `opencode web` not `serve`** — `serve` is API-only (empty body at `/`); `web` serves the browser UI. Residual: 0.0.0.0 bind = LAN-reachable bypassing CF Access, still password-gated (optional fw-scope follow-up). As-built fully documented in CLAUDE.md "Remote Agent Operations".
