...
Headless Daily News research routes built-in OMP `web_search` through SearXNG first via `~/.omp/agent/headless-override.yml`, with OMP's normal providers retained as fallback. Fresh-search health is tested against `categories=news&time_range=day`; an unfiltered general search is not sufficient because the working general engines do not currently honor the day filter.
