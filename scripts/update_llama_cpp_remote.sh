...
rollback() {
  ...
  if (( SWITCHED )); then
    ...
    if (( rollback_failed == 0 )) \
      && sudo -n systemctl is-active --quiet llama-swap.service \
      && curl -fsS --max-time 20 http://192.168.4.103:8080/v1/models >/dev/null; then
...
sudo -n systemctl is-active --quiet llama-swap.service
curl -fsS --max-time 20 http://192.168.4.103:8080/v1/models >/dev/null
