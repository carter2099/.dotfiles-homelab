#!/usr/bin/env bash
# Researches and emails the daily agentic platform digest via Pi + DeepSeek V4 Flash.
...
**Model:** deepseek-v4-flash | **Sent to:** carter2099@pm.me + CC
...
END_TS="$(date +%s)"
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) agentic-digest duration=$((END_TS - START_TS))s model=deepseek-v4-flash" >> "$HOME/digests/agentic-platform/.runs.log"
